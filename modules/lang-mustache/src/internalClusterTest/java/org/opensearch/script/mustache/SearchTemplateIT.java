...
public class SearchTemplateIT extends OpenSearchSingleNodeTestCase {
    ...
    public void setup() throws IOException {
        createIndex("test");
        client().prepareIndex("test", "_doc", "1").setSource(jsonBuilder().startObject().field("text", "value1").endObject()).get();
        client().prepareIndex("test", "_doc", "2").setSource(jsonBuilder().startObject().field("text", "value2").endObject()).get();
    ...
    public void testIndexedTemplateClient() throws Exception {
        ...
        bulkRequestBuilder.add(client().prepareIndex("test", "_doc", "1").setSource("{\"theField\":\"foo\"}", XContentType.JSON));
        bulkRequestBuilder.add(client().prepareIndex("test", "_doc", "2").setSource("{\"theField\":\"foo 2\"}", XContentType.JSON));
        bulkRequestBuilder.add(client().prepareIndex("test", "_doc", "3").setSource("{\"theField\":\"foo 3\"}", XContentType.JSON));
        bulkRequestBuilder.add(client().prepareIndex("test", "_doc", "4").setSource("{\"theField\":\"foo 4\"}", XContentType.JSON));
        bulkRequestBuilder.add(client().prepareIndex("test", "_doc", "5").setSource("{\"theField\":\"bar\"}", XContentType.JSON));
    ...
    public void testIndexedTemplateOverwrite() throws Exception {
        ...
        client().prepareIndex("testindex", "_doc", "1")
