...
public class DeleteByQueryBasicTests extends ReindexTestCase {
    ...
    public void testBasics() throws Exception {
        indexRandom(
            true,
            client().prepareIndex("test", "_doc", "1").setSource("foo", "a"),
            client().prepareIndex("test", "_doc", "2").setSource("foo", "a"),
            client().prepareIndex("test", "_doc", "3").setSource("foo", "b"),
            client().prepareIndex("test", "_doc", "4").setSource("foo", "c"),
            client().prepareIndex("test", "_doc", "5").setSource("foo", "d"),
            client().prepareIndex("test", "_doc", "6").setSource("foo", "e"),
            client().prepareIndex("test", "_doc", "7").setSource("foo", "f")
        );
    ...
    public void testDeleteByQueryWithOneIndex() throws Exception {
        ...
        for (int i = 0; i < docs; i++) {
            builders.add(client().prepareIndex("test", "_doc", String.valueOf(i)).setSource("fields1", 1));
        }
    ...
    public void testDeleteByQueryWithMultipleIndices() throws Exception {
        ...
        for (int i = 0; i < indices; i++) {
            ...
            for (int j = 0; j < docs; j++) {
                ...
                builders.add(client().prepareIndex("test-" + i, "_doc", String.valueOf(j)).setSource("candidate", candidate));
            }
        }
        indexRandom(true, true, true, builders);
    ...
    public void testDeleteByQueryWithMissingIndex() throws Exception {
        indexRandom(true, client().prepareIndex("test", "_doc", "1").setSource("foo", "a"));
    ...
    public void testDeleteByQueryWithRouting() throws Exception {
        ...
        List<IndexRequestBuilder> builders = new ArrayList<>();
        for (int i = 0; i < docs; i++) {
            builders.add(client().prepareIndex("test", "_doc", String.valueOf(i)).setRouting(String.valueOf(i)).setSource("field1", 1));
        }
        indexRandom(true, true, true, builders);
    ...
    public void testDeleteByQueryWithDateMath() throws Exception {
        indexRandom(true, client().prepareIndex("test", "_doc", "1").setSource("d", "2013-01-01"));
    ...
    public void testDeleteByQueryOnReadOnlyIndex() throws Exception {
        ...
        List<IndexRequestBuilder> builders = new ArrayList<>();
        for (int i = 0; i < docs; i++) {
            builders.add(client().prepareIndex("test", "_doc", Integer.toString(i)).setSource("field", 1));
        }
        indexRandom(true, true, true, builders);
    ...
    public void testDeleteByQueryOnReadOnlyAllowDeleteIndex() throws Exception {
        ...
        List<IndexRequestBuilder> builders = new ArrayList<>();
        for (int i = 0; i < docs; i++) {
            builders.add(client().prepareIndex("test", "_doc").setId(Integer.toString(i)).setSource("field", 1));
        }
        indexRandom(true, true, true, builders);
    ...
    public void testMultipleSources() throws Exception {
        ...
        for (int sourceIndex = 0; sourceIndex < sourceIndices; sourceIndex++) {
            ...
            for (int i = 0; i < numDocs; i++) {
                docs.get(indexName).add(client().prepareIndex(indexName, "_doc", Integer.toString(i)).setSource("foo", "a"));
            }
        }
